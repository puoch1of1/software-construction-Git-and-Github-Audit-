...
3. Git combines the histories and updates the files
4. If conflicts occur, you resolve them and commit
...
**Fast-forward merge**
Happens when the target branch has not changed since the feature branch was created. Git simply moves the branch pointer forward—no merge commit is created.

**No-fast-forward merge (`--no-ff`)**
Forces Git to create a merge commit even when a fast-forward is possible. This keeps branch history visible and is common in team projects.

**Squash merge (`--squash`)**
Combines all commits from a branch into a single commit before merging. Useful for keeping history clean.
